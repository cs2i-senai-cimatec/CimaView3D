...
| =points=     | List   | dicionário contendo ⟨x, y, z⟩          | None      |
...
| *Parameter*     | *Type*          | *Description*               | *Default*    |
|-----------------+-----------------+-----------------------------+--------------|
| =volume=        | np.ndarray      | 3D array                    | necessário   |
...
| =cbar_position= | str             | position of the colorbar    | to the right |
...
| =show_grid=   | True/False           | mostrar grade ao fundo do gráfico         | False      |
